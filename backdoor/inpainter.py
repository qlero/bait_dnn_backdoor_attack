"""
MODULE IMPLEMENTING THE INPAINTING GENERATOR USED BY THE BAIT ATTACK.
BASED ON: https://github.com/daa233/generative-inpainting-pytorch
"""
...
class FineGenerator(nn.Module):
    ...
    def forward(self, xin, x_stage1, mask):
        ...
        # For indicating the boundaries of images
        ones = torch.ones(xin.size(0), 1, xin.size(2), xin.size(3)).to(xin.device)
        # conv branch
...
class ContextualAttention(nn.Module):
    def __init__(self, ksize=3, stride=1, rate=1, fuse_k=3, softmax_scale=10, fuse=False, device="cpu"):
    ...
    def forward(self, f, b, mask=None):
        ...
        # process mask
        if mask is None:
            mask = torch.zeros([int_bs[0], 1, int_bs[2], int_bs[3]]).to(self.device)
        else:
            mask = F.interpolate(mask.float(), scale_factor=1./(4*self.rate), mode='nearest')
        ...
        flow           = flow.permute(0, 3, 1, 2).to(self.device)
        if self.rate != 1:
...
class Generator(nn.Module):
    """
    Generator class used by the BAIT backdoor attack.
    """
    ...
    def to(self, device):
        self.coarse_generator.to(device)
        self.fine_generator.to(device)
        self.fine_generator.contextul_attention.to(device)
